...
## Data Cleaning
Fitbit data were stored in two csv files: one for the participants' daily physical activity level and one for participants' nightly sleep duration. For the physical activity and sleep study, we imported two csv files: ???????? physical activity level and sleep duration in Jupyter Notebook. Once these files were imported, we put the data into a Pandas dataframe for cleaning.
